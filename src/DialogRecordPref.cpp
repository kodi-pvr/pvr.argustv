...
bool CDialogRecordPref::OnInit()
{
	...
	_spinRunType->AddLabel(XBMC->GetLocalizedString(30104), 0); // no options
	_spinRunType->AddLabel(XBMC->GetLocalizedString(30105), 1); // first runs only
	_spinRunType->AddLabel(XBMC->GetLocalizedString(30106), 2);	// episodes once
	_spinRunType->AddLabel(XBMC->GetLocalizedString(30109), 3);	// both first runs and episodes once
