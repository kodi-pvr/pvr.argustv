...
bool CSettings::Load()
{
  ...
  if (!kodi::CheckSettingString("host", m_szHostname))
  {
    ...
    kodi::Log(ADDON_LOG_ERROR, "Couldn't get 'host' setting, falling back to '127.0.0.1' as default");
  ...
  {
    /* If setting is unknown fallback to defaults */
    kodi::Log(ADDON_LOG_ERROR, "Couldn't get 'useradio' setting, falling back to 'true' as default");
  ...
  {
    /* If setting is unknown fallback to defaults */
    kodi::Log(ADDON_LOG_ERROR, "Couldn't get 'timeout' setting, falling back to %i seconds as default", DEFAULT_TIMEOUT);
  ...
  {
    /* If setting is unknown fallback to defaults */
    kodi::Log(ADDON_LOG_ERROR, "Couldn't get 'user' setting, falling back to '%s' as default", DEFAULT_USER);
  ...
  {
    /* If setting is unknown fallback to defaults */
    kodi::Log(ADDON_LOG_ERROR, "Couldn't get 'tunedelay' setting, falling back to '200' as default");
  ...
  {
    /* If setting is unknown fallback to defaults */
	kodi::Log(ADDON_LOG_ERROR, "Couldn't get 'usefolder' setting, falling back to 'false' as default");
...
ADDON_STATUS CSettings::SetSetting(const std::string& settingName, const kodi::CSettingValue& settingValue)
{
  if (settingName == "host")
  {
    std::string tmp_sHostname;
    kodi::Log(ADDON_LOG_INFO, "Changed Setting 'host' from %s to %s", m_szHostname.c_str(), settingValue.GetString().c_str());
  ...
  {
    kodi::Log(ADDON_LOG_INFO, "Changed Setting 'port' from %u to %u", m_iPort, settingValue.GetInt());
  ...
  {
    kodi::Log(ADDON_LOG_INFO, "Changed setting 'useradio' from %u to %u", m_bRadioEnabled, settingValue.GetBoolean());
    m_bRadioEnabled = settingValue.GetBoolean();
  }
  else if (settingName == "timeout")
  {
    kodi::Log(ADDON_LOG_INFO, "Changed setting 'timeout' from %u to %u", m_iConnectTimeout, settingValue.GetInt());
    m_iConnectTimeout = settingValue.GetInt();
  }
  else if (settingName == "user")
  {
    kodi::Log(ADDON_LOG_INFO, "Changed Setting 'user' from %s to %s", m_szUser.c_str(), settingValue.GetString().c_str());
    m_szUser = settingValue.GetString();
  }
  else if (settingName == "pass")
  {
    kodi::Log(ADDON_LOG_INFO, "Changed Setting 'pass' from %s to %s", m_szPass.c_str(), settingValue.GetString().c_str());
    m_szPass = settingValue.GetString();
  }
  else if (settingName == "tunedelay")
  {
    kodi::Log(ADDON_LOG_INFO, "Changed setting 'tunedelay' from %u to %u", m_iTuneDelay, settingValue.GetInt());
    m_iTuneDelay = settingValue.GetInt();
  }
  else if (settingName == "usefolder")
  {
    kodi::Log(ADDON_LOG_INFO, "Changed setting 'usefolder' from %u to %u", m_bUseFolder, settingValue.GetBoolean());
